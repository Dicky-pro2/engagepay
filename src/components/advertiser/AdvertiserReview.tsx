...
export default function AdvertiserReview() {
  ...
  const [rejectModal, setRejectModal] = useState<{ taskId: string; submissionId: string } | null>(null);
  const [rejectNote, setRejectNote] = useState('');

  // Flatten all submissions from all tasks
  const allSubmissions = useMemo(() => {
    return myTasks.flatMap((task) =>
      (task.taskSubmissions ?? []).map((sub) => ({
        ...sub,
        task,
      }))
  ...
  const handleReject = () => {
    ...
    notify.info('Submission rejected. Slot has been refunded to the task.');
    setRejectModal(null);
    setRejectNote('');
  };
          ...
          >
            ...
            >
              ...
              <p className="text-slatec text-sm mb-4">
                The slot will be refunded back to the task. Optionally leave a note for the earner.
              </p>
