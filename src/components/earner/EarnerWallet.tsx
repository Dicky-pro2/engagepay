...
const METHODS: { key: WithdrawalMethod; label: string; icon: string; placeholder: string }[] = [
  { key: 'bank_transfer', label: 'Bank Transfer', icon: '🏦', placeholder: 'Bank name, account number, account name' },
  { key: 'mobile_money', label: 'Mobile Money', icon: '📱', placeholder: 'Phone number and provider (e.g. MTN, Airtel)' },
  { key: 'paypal', label: 'PayPal', icon: '💳', placeholder: 'Your PayPal email address' },
  { key: 'crypto', label: 'Crypto (USDT)', icon: '₿', placeholder: 'Your USDT wallet address (TRC20)' },
];
...
export default function EarnerWallet() {
  ...
  const { transactions, withdrawals, addTransaction, addWithdrawal, pushActivity } = useAppStore();
  ...
  const earningTransactions = transactions.filter(
    (tx) => tx.type === 'task_earning' || tx.type === 'withdrawal' || tx.type === 'bonus'
  );
  ...
  const handleWithdraw = () => {
    ...
    addWithdrawal({ amount: amt, method, accountDetails: accountDetails.trim() });
    pushActivity(`Withdrawal of 🪙${amt.toLocaleString()} requested`, 'violet');
    notify.success(`Withdrawal of 🪙${amt.toLocaleString()} submitted! Processing soon.`);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slatec text-sm">Withdraw your earnings and track transactions.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        >
          <div className="text-xs text-slatec uppercase tracking-wide mb-1">Available to Withdraw</div>
        ...
        <motion.div
          ...
          className="card p-5"
        >
          <div className="text-xs text-slatec uppercase tracking-wide mb-1">Total Earned</div>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'withdraw' && (
          ...
          >
            <h2 className="font-sora font-bold text-base">Choose Withdrawal Method</h2>

            <div className="grid grid-cols-2 gap-2">
              {METHODS.map((m) => (
                <button
                  key={m.key}
                  onClick={() => { setMethod(m.key); setAccountDetails(''); }}
            ...
            <button
              ...
              disabled={!amount || Number(amount) < MIN_WITHDRAWAL || Number(amount) > (user?.walletBalance ?? 0)}
                ...
                {withdrawals.map((w) => {
                  ...
                  return (
                    <div key={w.id} className="flex items-center gap-3 px-4 sm:px-5 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium">{methodMeta.label}</div>
                        <div className="text-xs text-slatec truncate">{w.accountDetails}</div>
                        <div className="text-xs text-slatec">{new Date(w.createdAt).toLocaleString()}</div>
                      </div>
                      <div className="text-right">
                        <div className="font-sora font-bold text-sm text-red-400 mb-1">
                          -{w.amount.toLocaleString()} 🪙
                        </div>
                        <div className={`flex items-center gap-1 text-xs font-semibold ${statusMeta.color}`}>
