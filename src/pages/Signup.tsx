...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    notify.success(`Welcome to EngagePay, ${user.name}! 🎉`);
    setLoading(false);
    navigate('/dashboard');
  };
  ...
  return (
    <AuthShell>
      ...
      <p className="text-slatec text-sm mb-5">Choose how you'd like to use EngagePay.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <div className="relative">
            <Icons.User size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slatec pointer-events-none" />
            <input
...
<motion.div
      ...
      <p className="text-center text-sm text-slatec mt-6">
        ...
        <Link to="/login" className="text-violet-light font-semibold hover:underline">
          Log in
        </Link>
      </p>
      </motion.div>
  ...
  return (
    <button
      ...
      className={`flex flex-col items-center gap-1.5 rounded-xl border px-4 py-3 transition-all text-sm font-semibold ${
        active ? activeClasses[color] : 'border-border text-slatec hover:border-white/20'
      }`}
    >
      {icon}
      {label}
